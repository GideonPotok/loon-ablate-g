"""
Ablation G — CVaR α=0.75 on v1 config (mild risk-aversion):
  QR-DQN-51, cvar_alpha=0.75, v1 server (all v2 flags OFF).

Question this answers:
  "Can mild CVaR risk-aversion improve tropical/strong-shear scores
   without the 75% rollout-discard problem of α=0.25?"

v1 baseline used CVaR α=0.25 (top-25% quantiles, very conservative).
Ablation A showed plain DQN (n_q=1, effectively α=1.0) beats v1 by ~4pp.
This ablation tests the midpoint: α=0.75 keeps the distributional signal
but discards only the bottom 25% (mild pessimism, 3× less aggressive than v1).

Changes vs v1 baseline:
  - cvar_alpha: 0.25 → 0.75
  - N_WORKERS: 10 (same)
  - Everything else identical to v1 Phase 2

Usage:
    python ablate_g_train.py
"""
...
BASE_CONFIG = QRConfig(
    ...
    hidden_sizes      = [128, 64],
    action_count      = 17,
    n_quantiles       = 51,            # QR-DQN, same as v1
    huber_kappa       = 1.0,
    learning_rate     = 1e-4,
    optimizer         = 'adam',
    gamma             = 0.97,
    ...
    cvar_alpha        = 0.75,          # ← ABLATION: mild risk-aversion (was 0.25)
...
LOG_PATH       = Path('/tmp/train_ablate_g.log')
WEIGHTS_PREFIX = 'dqn_ablate_g'
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION G: CVaR α=0.75, QR-DQN-51, v1 server')
    tee('═' * 78)
    tee(f'Workers:     {N_WORKERS}')
    tee(f'Network:     {BASE_CONFIG.state_dim} → {" → ".join(str(h) for h in BASE_CONFIG.hidden_sizes)} '
        f'→ {BASE_CONFIG.action_count * BASE_CONFIG.n_quantiles} '
        f'({BASE_CONFIG.action_count} actions × {BASE_CONFIG.n_quantiles} quantiles)   '
        f'({n_params:,} params)')
    tee('Curriculum:  ' + '  →  '.join(
        f'{t["label"]}×{t["episodes"]}' for t in CURRICULUM) + f'   total {TOTAL_EPS} eps/worker')
    tee(f'CVaR α:      {BASE_CONFIG.cvar_alpha}  (v1 was 0.25, ablate_a was 1.0/n_q=1)')
    tee(f'n_quantiles: {BASE_CONFIG.n_quantiles}')
    tee(f'Server:      v1 (all v2 flags OFF)')
    tee('─' * 78)
